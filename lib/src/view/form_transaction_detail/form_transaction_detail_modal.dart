...
class FormTransactionDetailModal extends ConsumerWidget {
  const FormTransactionDetailModal({
    Key? key,
    required this.id,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    /// Listen [transactionDetailNotifier]
    ref.listen<AsyncValue<TransactionDetailModel?>>(
      transactionDetailNotifier(id).select((value) => value.item),
      (_, state) {
        /// Ketika berhasil load detail transaksi
        /// Setup [form transaction detail parameter]
        state.whenOrNull(
          data: (item) {
            ref.watch(formTransactionDetailParameter.notifier).update(
    ...
    ref.listen<AsyncValue>(
      transactionDetailActionNotifier.select((value) => value.insertOrUpdate),
      (_, state) {
    ...
    final _state = ref.watch(transactionDetailNotifier(id)).item;
    return _state.when(
      data: (_) {
        return AlertDialog(
          scrollable: true,
          ...
          actions: [
            ElevatedButton(
              onPressed: () async {
                final form = ref.read(formTransactionDetailParameter);
                await ref
                    .read(transactionDetailActionNotifier.notifier)
                    .insertOrUpdateTransactionDetail(form);
              },
